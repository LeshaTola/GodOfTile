...
namespace TileSystem
{
	internal class TileGrid : MonoBehaviour
	{
			...
			for (int x = 0; x < activeTile.Config.Size.x; x++)
			{
				for (int y = 0; y < activeTile.Config.Size.y; y++)
				{
					Vector2Int tileCoordinate
						= new((int)activeTile.transform.position.x + x,
						(int)activeTile.transform.position.z + y);
		...
		{
			bool result;
			result = !(tile.transform.position.x > maxGridSize.x - tile.Config.Size.x
					|| tile.transform.position.z > maxGridSize.y - tile.Config.Size.y)
